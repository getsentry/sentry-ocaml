...
module Transport = struct
  ...
  let create dsn_info =
    let endpoint = Printf.sprintf "%s/api/%s/envelope/" dsn_info.base_uri dsn_info.project_id in
    { endpoint; dsn_info }
  ;;
  ...
  let build_envelope_header (event : Event.t) =
    ...
    let header_fields = [ ("event_id", `String event.event_id); ("sent_at", `String sent_at) ] in
    `Assoc header_fields
  ;;
