...
module Context = struct
  ...
  let set_user context user = 
    { context with user = Some user }
  ...
  let set_environment context env =
    { context with environment = Some env }
  
  let set_release context release =
    { context with release = Some release }
  ...
  let to_json context =
    ...
    let fields =
      if context.tags <> [] then
        let tags_json = 
          List.map (fun (k, v) -> (k, `String v)) context.tags
        in
    ...
    let fields =
      if context.extra <> [] then
        let extra_json = 
          List.map (fun (k, v) -> (k, `String v)) context.extra
        in
